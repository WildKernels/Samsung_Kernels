		...
		struct drm_atomic_state *state)
{
	...
	SDE_ATRACE_BEGIN("check_vm_request");
	ret = sde_kms_check_vm_request(kms, state);
	if (ret) {
		SDE_ERROR("vm switch request checks failed\n");
		SDE_ATRACE_END("check_vm_request");
		goto end;
	}
	SDE_ATRACE_END("check_vm_request");

	SDE_ATRACE_BEGIN("helper_check");
	ret = drm_atomic_helper_check(dev, state);
	if (ret) {
		SDE_ATRACE_END("helper_check");
		goto vm_clean_up;
	}
	SDE_ATRACE_END("helper_check");
	/*
	 ...
	 * Secure state
	 */
	SDE_ATRACE_BEGIN("check_secure");
	ret = sde_kms_check_secure_transition(kms, state);
	if (ret) {
		SDE_ATRACE_END("check_secure");
		goto vm_clean_up;
	}
	SDE_ATRACE_END("check_secure");

	SDE_ATRACE_BEGIN("check_cwb");
	ret = sde_kms_check_cwb_concurreny(kms, state);
	if (ret) {
		SDE_ATRACE_END("check_cwb");
		goto vm_clean_up;
	}
	SDE_ATRACE_END("check_cwb");
