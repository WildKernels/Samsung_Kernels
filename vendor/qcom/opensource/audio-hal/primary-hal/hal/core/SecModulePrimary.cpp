...
// SEC_GENERIC
            ...
            {SecParameters::kAuracastRemoteMicEnable, SecParameters::Feature_SEC::SEC_GENERIC},
...
void SecModulePrimary::onSetSECGenericParameters(const std::vector<VendorParameter>& params) {
    for (const auto& param : params) {
        ...
        else if (SecParameters::kAuracastRemoteMicEnable == param.id) {
            bool enableArm = getBoolFromString(paramValue);
            mPlatform.setArmEnabled(enableArm);
    ...
    for (const auto& p : parameters) {
        ...
        if (SecParameters::kVoiceCallState == p.id) {
            ...
            if (mPlatform.getCallTranslation() != enable) {
                ...
                mAudioEffect.SetVoiceRxEffectForTranslation(__func__, enable);
            }
...
void SecModulePrimary::SetFolderState(int state) {
    ...
    if (prevFolderclosed != mPlatform.getFolderclosed()) {
        mAudExt.mArmExtension->setFlipClosed(mPlatform.getFolderclosed());
        mAudExt.mArmExtension->updateArmStream();
    }
#endif
}
