...
class AudioExtensionBase {
  public:
    ...
    static void setBleBroadcastDeviceConnected(bool connected) {
        mBleBroadcastDeviceConnected = connected;
    }
    static bool isBleBroadcastDeviceAvailable() { return mBleBroadcastDeviceConnected; }
    static void setUsbInputDeviceConnected(const pal_usb_device_address& addr, bool connected) {
        mUsbInputDeviceConnected = connected; 
        mUsbInputDeviceAddr = connected ? addr : kUsbInputDeviceAddrInitializer; }
    static bool isUsbInputDeviceAvailable() { 
        return mUsbInputDeviceConnected && mUsbInputDeviceAddr.card_id >= 0 && 
            mUsbInputDeviceAddr.device_num >= 0; }
    static const pal_usb_device_address& getUsbInputDeviceAddress() { return mUsbInputDeviceAddr; }
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
    static void setFlipClosed(bool closed) { mFlipClosed = closed; }
#endif
#endif
  protected:
...
#endif
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
    static constexpr pal_usb_device_address kUsbInputDeviceAddrInitializer = { -1, -1, };
    static bool mBleBroadcastDeviceConnected;
    static bool mUsbInputDeviceConnected;
    static struct pal_usb_device_address mUsbInputDeviceAddr;
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
    static bool mFlipClosed;
#endif
#endif
...
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
class ArmExtension : public AudioExtensionBase {
  public:
    ArmExtension();
    ~ArmExtension();
    bool isValidStatusForArm();
    void clearArmStream();
    int startArmStream(pal_device_id_t inDeviceId);
    int stopArmStream();
    int updateArmStream();
    void updateUnsupportedDevicesRouted(const std::vector<pal_device>& devices);
    bool isUnsupportedDevicesRouted() const { return mUnsupportedDevicesRouted; }
  protected:
    bool isArmActive() { return (mArmStreamHandle != nullptr) ? true : false; }
    pal_device_id_t mArmInDeviceId;
    pal_stream_handle_t *mArmStreamHandle;
    bool mUnsupportedDevicesRouted;
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
    bool mStartInFlipClosed;
#endif
};
#endif
...
#endif
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
    std::unique_ptr<ArmExtension> mArmExtension = std::make_unique<ArmExtension>();
#endif
#ifdef SEC_AUDIO_DSM_AMP
