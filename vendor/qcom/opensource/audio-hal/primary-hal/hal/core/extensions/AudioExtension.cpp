...
/* static */ bool AudioExtensionBase::mBleBroadcastDeviceConnected = false;
/* static */ bool AudioExtensionBase::mUsbInputDeviceConnected = false;
/* static */ struct pal_usb_device_address AudioExtensionBase::mUsbInputDeviceAddr = 
        AudioExtensionBase::kUsbInputDeviceAddrInitializer;
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
/* static */ bool AudioExtensionBase::mFlipClosed = false;
#endif
#endif
...
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
bool ArmExtension::isValidStatusForArm() {
    // 1. check call mode.
    switch (static_cast<audio_mode_t>(mPlatform.getCallMode())) {
    case AUDIO_MODE_IN_CALL:
    case AUDIO_MODE_IN_COMMUNICATION:
    case AUDIO_MODE_CALL_SCREEN:
        /* on - support only Normal Mode.  off - working all audio mode */
        LOG(ERROR) << __func__ << ": Not support ARM during call mode";
        return false;
    ...
    // 2. check enabled by setParameters.
    if (!mPlatform.isArmEnabled()) {
        return false;
    }

    // 3. check ble broadcast device connected.
    if (!AudioExtensionBase::mBleBroadcastDeviceConnected) {
        return false;
    }

    // 4. check if another out device is playing.
    if (mUnsupportedDevicesRouted) {
        return false;
    }
...
int ArmExtension::updateArmStream() {
    const bool validStatus = isValidStatusForArm();
    int ret = 0;

    LOG(DEBUG) << __func__ << " Enter : validStatus " << validStatus;

    if (validStatus) {
        const pal_device_id_t inDeviceId = 
            isUsbInputDeviceAvailable() ? PAL_DEVICE_IN_USB_HEADSET : PAL_DEVICE_IN_HANDSET_MIC;

        if (isArmActive()) {
            if (inDeviceId != mArmInDeviceId) {
                stopArmStream();
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
            } else if (inDeviceId == PAL_DEVICE_IN_HANDSET_MIC
                    && mFlipClosed != mStartInFlipClosed) {
                stopArmStream();
#endif
            } else {
                LOG(DEBUG) << __func__ << ": Arm is already active. skip to start ARM stream";
                return ret;
            }
        }

        ret = startArmStream(inDeviceId);
        if (ret) {
            LOG(ERROR) << __func__ << ": Failed to start ARM stream ret = " << ret;
            stopArmStream();
        } else {
            LOG(DEBUG) << __func__ << ": Start ARM stream success";
        }
    }  else {
        if (isArmActive()) {
            ret = stopArmStream();
        } else {
            LOG(DEBUG) << __func__ << ": ARM is not active state. skip to stop ARM stream";
            clearArmStream();
        }
    }
    LOG(DEBUG) << __func__ << " Exit: ret " << ret;
...
int ArmExtension::startArmStream(pal_device_id_t inDeviceId)
{
    static constexpr size_t kNumPalDevs = 2;
    static constexpr size_t kIndexOutDev = 0;
    static constexpr size_t kIndexInDev = 1;
    static constexpr uint32_t kSampleRate = DEFAULT_OUTPUT_SAMPLING_RATE;

    struct pal_device palDevs[kNumPalDevs];
    struct pal_stream_attributes armStreamAttributes;
    pal_channel_info chInfo = {0, {0}};
    int ret = 0;

    chInfo.channels = 1;
    chInfo.ch_map[0] = PAL_CHMAP_CHANNEL_FL;

    memset(&armStreamAttributes, 0, sizeof(armStreamAttributes));
    armStreamAttributes.type = PAL_STREAM_LOOPBACK;
    armStreamAttributes.info.opt_stream_info.loopback_type = PAL_STREAM_LOOPBACK_KARAOKE;
    armStreamAttributes.direction = PAL_AUDIO_INPUT_OUTPUT;
    armStreamAttributes.in_media_config.sample_rate = kSampleRate;
    armStreamAttributes.in_media_config.bit_width = CODEC_BACKEND_DEFAULT_BIT_WIDTH;
    armStreamAttributes.in_media_config.ch_info = chInfo;
    armStreamAttributes.in_media_config.aud_fmt_id = PAL_AUDIO_FMT_DEFAULT_PCM;
    armStreamAttributes.out_media_config.sample_rate = kSampleRate;
    armStreamAttributes.out_media_config.bit_width = CODEC_BACKEND_DEFAULT_BIT_WIDTH;
    armStreamAttributes.out_media_config.ch_info = chInfo;
    armStreamAttributes.out_media_config.aud_fmt_id = PAL_AUDIO_FMT_DEFAULT_PCM;

    auto initPalDev = [&](pal_device& palDevice) {
        memset(palDevice.custom_config.custom_key, 0, PAL_MAX_CUSTOM_KEY_SIZE);
        palDevice.config.sample_rate = kSampleRate;
        palDevice.config.bit_width = CODEC_BACKEND_DEFAULT_BIT_WIDTH;
        palDevice.config.ch_info = chInfo;
        palDevice.config.aud_fmt_id = PAL_AUDIO_FMT_DEFAULT_PCM;
    };

    // out device
    palDevs[kIndexOutDev].id = PAL_DEVICE_OUT_BLUETOOTH_BLE_BROADCAST;
    initPalDev(palDevs[kIndexOutDev]);
    strcpy(palDevs[kIndexOutDev].custom_config.custom_key, ck_table[CUSTOM_KEY_AURACAST_REMOTE_MIC]);
    LOG(INFO) << __func__ << " Setting custom key for out pal device : " 
            << palDevs[kIndexOutDev].custom_config.custom_key;

    // in device
    palDevs[kIndexInDev].id = inDeviceId; 
    initPalDev(palDevs[kIndexInDev]);
    strcpy(palDevs[kIndexInDev].custom_config.custom_key, ck_table[CUSTOM_KEY_AURACAST_REMOTE_MIC]);
    LOG(INFO) << __func__ << " Setting custom key for in pal device : " 
            << palDevs[kIndexInDev].custom_config.custom_key;
    if (inDeviceId == PAL_DEVICE_IN_USB_HEADSET) {
        palDevs[kIndexInDev].address = getUsbInputDeviceAddress();
    }

    ret = pal_stream_open(&armStreamAttributes,
            kNumPalDevs, palDevs,
            0,
            nullptr,
            nullptr /* callback */,
    ...
    if (ret) {
        LOG(ERROR) << __func__ << ": Failed to open ARM stream ret = " << ret;
        clearArmStream();
        return ret;
    }

    LOG(DEBUG) << __func__ << ": ARM stream open success";
    mArmInDeviceId = inDeviceId;
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
    if (inDeviceId == PAL_DEVICE_IN_HANDSET_MIC) {
        bool flipClosed = mFlipClosed;
        ret = pal_set_param(PAL_PARAM_ID_FLIP_CLOSED, &flipClosed, sizeof(bool));
        if (ret) {
            LOG(WARNING) << __func__ << ": Failed to set parameter flip closed ret = " << ret;
            mStartInFlipClosed = false;
        } else {
            mStartInFlipClosed = flipClosed;
        }
    }
#endif
    return pal_stream_start(mArmStreamHandle);
}

int ArmExtension::stopArmStream()
{
    int ret = pal_stream_stop(mArmStreamHandle);
    if (ret) {
        LOG(ERROR) << __func__ << ": Failed to stop ARM Stream ret = " << ret;
    }

    ret = pal_stream_close(mArmStreamHandle);
    if (ret) {
        LOG(ERROR) << __func__ << ": Failed to close ARM stream ret = " << ret;
    } else {
        LOG(DEBUG) << __func__ << ": ARM stream close success";
    }
    clearArmStream();
    return ret;
}

void ArmExtension::clearArmStream()
{
    mArmInDeviceId = PAL_DEVICE_NONE;
    mArmStreamHandle = nullptr;
#ifdef SEC_AUDIO_SUPPORT_RCV_FLIP_CALL
    mStartInFlipClosed = false;
#endif
}

void ArmExtension::updateUnsupportedDevicesRouted(const std::vector<pal_device>& devices)
{
    for (const auto& device : devices) {
        if (device.id != PAL_DEVICE_OUT_BLUETOOTH_BLE_BROADCAST) {
            LOG(ERROR) << __func__ << ": unsupported device " << device.id << " routed";
            mUnsupportedDevicesRouted = true;
            return;
        }
    }
    mUnsupportedDevicesRouted = false;
}

ArmExtension::~ArmExtension() {}
ArmExtension::ArmExtension() : AudioExtensionBase(kDummyLibrary) {
    LOG(INFO) << __func__ << " Enter";
    mArmInDeviceId = PAL_DEVICE_NONE;
    mArmStreamHandle = nullptr;
    mUnsupportedDevicesRouted = false;
