...
#if defined(SEC_AUDIO_CALL_TRANSLATION) && defined(SEC_AUDIO_ADD_FOR_DEBUG)
void Platform::updateTranslationMode(const char *caller, bool mode, int ret)
{
    mLastTranslationMode = mode;
    mTranslationModeLog.log("set PARAM_ID_DV_TRANSLATE_MODE as %d, ret %s by caller %s\n",
                            mode, (ret != 0) ? "FAILED" : "SUCCESS", caller);
}
#endif
...
int Platform::handleDeviceConnectionChange(const AudioPort& deviceAudioPort,
    ...
    if (isUsbDevice(devicePortExt.device)) {
        ...
        if (isInputDevice(devicePortExt.device)) {
            AudioExtensionBase::setUsbInputDeviceConnected(deviceConnection->device_config.usb_addr, isConnect);
        }
#endif
...
#endif
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
    if (devicePortExt.device.type.type == AudioDeviceType::OUT_BROADCAST &&
            devicePortExt.device.type.connection == AudioDeviceDescription::CONNECTION_BT_LE) {
        AudioExtensionBase::setBleBroadcastDeviceConnected(isConnect);
    }
#endif
...
std::string Platform::toStringSec() const {
...
#endif
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
    os << "  mArmEnabled : " << (mArmEnabled? "on" : "off") << std::endl;
#endif
    ...
    os << "  mLastTranslationMode : " << (mLastTranslationMode? "on" : "off") << std::endl;
#endif
    os << std::endl;
    return os.str();
}
...
#ifdef SEC_AUDIO_CALL_TRANSLATION
    auto dumpLoggerTranslationMode = [fd](const android::SimpleLog& logger) {
        dprintf(fd, "\nCall Translation Mode Setting History:\n");
        logger.dump(fd, "    " /* prefix */);
    };
    dumpLoggerTranslationMode(mTranslationModeLog);
#endif
    pal_dump(fd);
}
#endif
