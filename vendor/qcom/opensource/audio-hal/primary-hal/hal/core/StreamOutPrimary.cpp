...
ndk::ScopedAStatus StreamOutPrimary::configureConnectedDevices_I() {
    ...
    if (mTag == Usecase::PRIMARY_PLAYBACK) {
        ...
        if (streamOut && (streamOut->HasPalStreamHandle() ||
            ...
            if (mPlatform.getCallTranslation()) {
                mAudioEffect.SetVoiceRxEffectForTranslation(__func__, true);
            }
#endif
        }
    }
    ...
    if (mTag == Usecase::PRIMARY_PLAYBACK) {
        bool wasUnsupportedDevicesRouted = mAudExt.mArmExtension->isUnsupportedDevicesRouted();
        mAudExt.mArmExtension->updateUnsupportedDevicesRouted(mPlatform.convertToPalDevices(mConnectedDevices));
        if (wasUnsupportedDevicesRouted != mAudExt.mArmExtension->isUnsupportedDevicesRouted()) {
            mAudExt.mArmExtension->updateArmStream();
        }
    }
#endif
...
::android::status_t StreamOutPrimary::standby() {
    ...
    if (mTag == Usecase::VOIP_PLAYBACK) {
        ...
        if (mPlatform.getCallTranslation() && mPlatform.getCallState() != 2 /*CallState::ACTIVE*/) {
            mAudioEffect.SetVoiceRxEffectForTranslation(__func__, false);
        }
#endif
    }
#endif
