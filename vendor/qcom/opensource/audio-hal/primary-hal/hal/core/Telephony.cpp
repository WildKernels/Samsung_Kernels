...
ndk::ScopedAStatus Telephony::switchAudioMode(AudioMode newAudioMode) {
    ...
    if (newAudioMode == AudioMode::IN_CALL
            || newAudioMode == AudioMode::CALL_SCREEN)  {
        // to avoid adsp crash when switching from CALL_SCREEN to IN_CALL
        if ((newAudioMode != mAudioMode) &&
            mAudioMode ==  AudioMode::CALL_SCREEN) {
            VoiceStop();
        }
...
ndk::ScopedAStatus Telephony::startCall() {
...
#endif
    if (mAudioMode == AudioMode::CALL_SCREEN) {
        LOG(DEBUG) << __func__ << " CALL SCREEN update sec pal devices";
        palDevices[0].id = PAL_DEVICE_OUT_PROXY;  //overwrite the device with proxy dev
        palDevices[1].id = PAL_DEVICE_IN_PROXY;  //overwrite the device with proxy dev
    }
#endif
    ...
    if (mAudioMode != AudioMode::CALL_SCREEN) {
        mAudioEffect.SetVoiceRxEffectForTranslation(__func__,
                                                    mPlatform.getCallTranslation());
    }
#endif
