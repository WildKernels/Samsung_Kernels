...
int ModulePrimary::onExternalDeviceConnectionChanged(
    ...
    const bool wasUsbInputAvailable = mAudExt.mArmExtension->isUsbInputDeviceAvailable();
    const bool wasBleBroadcastAvailable = mAudExt.mArmExtension->isBleBroadcastDeviceAvailable();
#endif
...
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
    else if (wasUsbInputAvailable != mAudExt.mArmExtension->isUsbInputDeviceAvailable() ||
            wasBleBroadcastAvailable != mAudExt.mArmExtension->isBleBroadcastDeviceAvailable()) {
