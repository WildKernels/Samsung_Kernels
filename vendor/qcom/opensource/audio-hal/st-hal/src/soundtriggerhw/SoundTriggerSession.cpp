                        ...
                        const std::shared_ptr<ISoundTriggerHwCallback> &stHwCallback)
{
    ...
    mActiveCallbackThreads = std::make_shared<std::atomic<int>>(0);
#endif
}
...
SoundTriggerSession::~SoundTriggerSession()
{
    ...
    if (wasShutdown) {
        STHAL_WARN(LOG_TAG, "Shutdown flag already set for handle %d (duplicate destructor call?)", mSessionHandle);
    }

    int activeThreadsAtShutdown = 0;
    if (mActiveCallbackThreads) {
        activeThreadsAtShutdown = mActiveCallbackThreads->load();
    }

    if (activeThreadsAtShutdown > 0) {
        STHAL_INFO(LOG_TAG, "Destructor: %d callback threads still running for handle %d",
                    activeThreadsAtShutdown, mSessionHandle);
    }
#endif
}
    ...
    struct pal_st_phrase_recognition_event *palPhraseEvent)
{
    ...
    STHAL_INFO(LOG_TAG, "sending %s", event.toString().c_str());
#ifdef SEC_AUDIO_SOUND_TRIGGER_TYPE
    SoundModelHandle handle = getSessionHandle();
    std::shared_ptr<ISoundTriggerHwCallback> callback = mClientCallback;
#endif
    mSessionMutex.unlock();
#ifdef SEC_AUDIO_SOUND_TRIGGER_TYPE
    createCallbackThread([callback, handle, event = std::move(event)]() {
        STHAL_INFO(LOG_TAG, "Processing phrase recognition callback for handle %d", handle);
        if (callback) {
            callback->phraseRecognitionCallback(handle, event);
        } else {
            STHAL_WARN(LOG_TAG, "Callback is null for handle %d", handle);
        }
    });
#else
...
#ifdef SEC_AUDIO_SOUND_TRIGGER_TYPE
void SoundTriggerSession::createCallbackThread(std::function<void()> callback)
{
    if (!callback) {
        STHAL_WARN(LOG_TAG, "Callback is null/empty for handle %d, skipping", mSessionHandle);
        return;
    }
    ...
    bool hasCounter = (mActiveCallbackThreads != nullptr);

    if (hasCounter) {
        currentThreadCount = mActiveCallbackThreads->load();
    } else {
        STHAL_WARN(LOG_TAG, "mActiveCallbackThreads is null for handle %d", mSessionHandle);
        return;
    }

    if (currentThreadCount >= MAX_CONCURRENT_THREADS) {
        STHAL_WARN(LOG_TAG, "Thread limit reached (%d/%d) for handle %d, skipping callback",
                    currentThreadCount, MAX_CONCURRENT_THREADS, mSessionHandle);
        return;
    }

    auto counterPtr = mActiveCallbackThreads;
    std::thread([counterPtr, callback = std::move(callback), handle = mSessionHandle]() {
        struct ThreadGuard {
            std::shared_ptr<std::atomic<int>> counter;
            int handle;

            ThreadGuard(std::shared_ptr<std::atomic<int>> c, int h)
                : counter(c), handle(h) {
                if (counter) {
                    counter->fetch_add(1);
                    STHAL_INFO(LOG_TAG, "Thread started for handle %d: count: %d", handle, counter->load());
                } else {
                    STHAL_WARN(LOG_TAG, "ThreadGuard: counter is null for handle %d", handle);
                }
            }
            ...
            ~ThreadGuard() {
                if (counter) {
                    counter->fetch_sub(1);
                    STHAL_INFO(LOG_TAG, "Thread ended for handle %d: count: %d", handle, counter->load());
                } else {
                    STHAL_WARN(LOG_TAG, "ThreadGuard destructor: counter is null for handle %d", handle);
                }
            }
        } guard(counterPtr, handle);

        if (callback) {
            callback();
        } else {
            STHAL_WARN(LOG_TAG, "Callback is empty inside thread for handle %d", handle);
        }
    }).detach();
    STHAL_INFO(LOG_TAG, "Exit, handle=%d", mSessionHandle);
}
#endif
