...
class BazelBuilder:
    ...
    def write_sec_localversion(
        ...
        code_version = os.environ.get('CL_SYNC')
        target_build_variant = os.environ.get('TARGET_BUILD_VARIANT')
        build_number = os.environ.get('BUILD_NUMBER')

        # Use sha256sum of common kernel codes instead of CL_SYNC
        if os.path.isfile(os.path.join(self.workspace, 'common_hash')):
            with open(os.path.join(self.workspace, 'common_hash'), 'r') as file:
                content = file.read().strip()
                if len(content) >= 7:
                    code_version = 'p' + content[:7]

        localversion = "-{}-abogki{}".format(code_version, build_number)
        if target_build_variant == 'userdebug':
            localversion = "-{}-{}-abogki{}".format(code_version, "ud", build_number)
        elif target_build_variant == 'eng':
            localversion = "-{}-{}-abogki{}".format(code_version, "en", build_number)
