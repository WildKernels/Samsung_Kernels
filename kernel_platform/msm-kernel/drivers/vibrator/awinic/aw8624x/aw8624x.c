 ...
 *
 ******************************************************/
#define AW8624X_DRIVER_VERSION "v0.0.0.18"
						 ...
						 int file_lens, int rtp_cali_len)
{
	...
	int *stop_pos = NULL;
	int vib_judg_cnt = 0;
	int zero_judg_cnt = aw8624x->replace_zero_cnt;
	int seg_src_len = 0;
	int seg_dst_len = 0;
	...
	for (i = 0; i < file_lens; i++) {
		...
		for (j = i + 1; j < file_lens; j++) {
			...
			if (src_data[j] == 0 || src_data[j] == 0x80) {
				...
				ret = aw8624x_rtp_zero_data_des(&src_data[j + aw8624x->add_cnt],
						file_lens, zero_judg_cnt);
#ifdef AW_ADD_BRAKE_WAVE
				if (ret == 1 && en_replace && aw8624x->dts_info.is_enabled_inter_brake) {
					aw_info("index : %d ", index);
					en_replace = false;
					memset(&src_data[j + aw8624x->add_cnt], 0x80, sizeof(uint8_t) * zero_judg_cnt);
				}
#endif
				if (!ret)
					continue;
...
static ssize_t add_zero_store(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
	...
	if (val > 0) {
		aw8624x->add_cnt = val;
		aw_info("aw8624x->add_cnt:%d", aw8624x->add_cnt);
	} else {
		aw_err("error input param");
	}
	mutex_unlock(&aw8624x->lock);
...
static ssize_t add_zero_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw8624x *aw8624x = aw8624x_get_drvdata(dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "aw8624x->add_cnt = %d\n",
					aw8624x->add_cnt);

	return len;
}

static ssize_t replace_zero_cnt_store(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
	...
	mutex_lock(&aw8624x->lock);
	if (val >= AW_MIN_REPLACE_ZERO) {
		aw8624x->replace_zero_cnt = val;
		aw_info("aw8624x->replace_zero_cnt:%d", aw8624x->replace_zero_cnt);
	} else {
		aw_err("error input param");
	}
	mutex_unlock(&aw8624x->lock);
...
static ssize_t replace_zero_cnt_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw8624x *aw8624x = aw8624x_get_drvdata(dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "aw8624x->replace_zero_cnt = %d\n",
					aw8624x->replace_zero_cnt);
...
static DEVICE_ATTR_RW(add_zero);
static DEVICE_ATTR_RW(replace_zero_cnt);
...
static struct attribute *vibrator_attributes[] = {
	...
	&dev_attr_add_zero.attr,
	&dev_attr_replace_zero_cnt.attr,
	NULL
};
...
#endif
	aw8624x->add_cnt = AW_DEAULT_ADD_CNT;
	aw8624x->replace_zero_cnt = AW_REPLACE_ZERO;
	aw8624x->input_flag = false;
