...
void synaptics_ts_send_rawdata(struct synaptics_ts_data *ts, u8 *data, bool is_released)
{
	...
	val = (s16 *)data;
	for (ii = 0; ii < ts->rawdata_buf_size; ii++) {
			ts->raw[4 + ii] = val[ii];
	}
	ts->raw[2] = palm;
	...
	sec_input_rawdata_copy_to_user(ts->raw, ts->rawdata_buf_size + (POSTFIX_BYTE_LENGTH + PREFIX_BYTE_LENGTH), press);
}
...
irqreturn_t synaptics_ts_irq_thread(int irq, void *ptr)
{
	...
	ret = event_id = curr_pos = remain_event_count = raw_buf_pos = 0;
	...
	if (is_raw_event) {
		...
		raw_buf_pos = 4 + remain_event_count * sizeof(struct sec_touch_event_data) + 2;
		raw = &ts->event_data.buf[raw_buf_pos];

		if (ts->event_data.buf_size < raw_buf_pos + ts->rawdata_buf_size) {
			is_raw_event = false;
			input_err(true, ts->dev, "%s: event_data alloc buf_size (%d) rawdata_size (%d)\n",
				__func__, ts->event_data.buf_size, raw_buf_pos + ts->rawdata_buf_size);
			goto exit;
		}
	} else {
