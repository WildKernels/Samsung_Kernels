...
void synaptics_ts_check_dualization_ic(struct synaptics_ts_data *ts)
{
	int masked_lcd_type = ts->plat_data->lcd_type & 0xffff00;
	int dualization_lcd_id[2] = {0x804000, 0x819400}; // 0x804000 : S3908T, 0x819400 : S3916T for B5 dualization

	ts->is_dualization_ic = false;

	if ((masked_lcd_type == dualization_lcd_id[0]) || (masked_lcd_type == dualization_lcd_id[1]))
		ts->is_dualization_ic = true;

	input_info(true, ts->dev, "%s, lcd_type : 0x%06X, is_dualization_ic : %d\n", __func__, ts->plat_data->lcd_type, ts->is_dualization_ic);
}

void synaptics_ts_change_dualization_afe_base(struct synaptics_ts_data *ts)
{
	struct device_node *np = ts->dev->of_node;

	if (of_property_read_u32(np, "synaptics,dualization_afe_base", &ts->tdata->afe_base) < 0) {
		ts->tdata->afe_base = 0;
		input_err(true, ts->dev, "%s: Failed to get dualization dualization_afe_base property\n", __func__);
	}

	input_info(true, ts->dev, "%s, change afe_base for dualization ic. afe_base: 0x%04X\n", __func__, ts->tdata->afe_base);
}
...
static int synaptics_ts_hw_init(struct synaptics_ts_data *ts)
{
	...
	synaptics_ts_check_dualization_ic(ts);

	if (ts->is_dualization_ic && (ts->tdata->tclm_level != TCLM_LEVEL_NOT_SUPPORT))
		synaptics_ts_change_dualization_afe_base(ts);
	...
	if (ts->is_dualization_ic)
		ret = synaptics_ts_fw_update_on_probe_opt(ts);
	else
