...
#if IS_ENABLED(CONFIG_SSC_SMP2P_EVENT_DEBUG)
#define SMP2P_CHECK_SKIP_TIME		(100000000LL)	//100s
...
static void qcom_smp2p_sns_event_check(struct work_struct *work)
{
	...
	uint64_t usec_system = ktime_to_us(ktime_get_boottime());

	if (usec_system < SMP2P_CHECK_SKIP_TIME) {
		dev_info(smp2p->dev, "skip smp2p check:%d\n",
			(int)(usec_system/1000000));
		return;
	}
