...
static int sec_ap_pmic_probe(struct platform_device *pdev)
{
	...
	/* Set delay time for starting ws_work (60s) */
	info->ws_start_delay = WS_START_DELAY;
	...
	schedule_delayed_work(&info->ws_work, info->ws_start_delay * HZ);
