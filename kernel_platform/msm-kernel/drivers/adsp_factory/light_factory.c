...
#endif
	if (!strncmp(data->light_device_vendor[display_idx], NO_DEVICE_STR, DEVICE_INFO_LENGTH)) {
		if (data->light_device_name_retries[display_idx] <= 3) {
			data->light_device_name_retries[display_idx]++;
			light_get_device_id(data, light_idx);
		}
		return;
	}
...
void light_init_work(struct adsp_data *data)
{
	...
	data->light_device_name_retries[MAIN_DISPLAY_IDX] = 0;
	data->light_device_name_retries[SUB_DISPLAY_IDX] = 0;
