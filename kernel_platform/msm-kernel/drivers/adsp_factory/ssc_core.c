...
#define TIME_LEN	24
#define WAKELOCK_DURATION_MAX	(SMP2P_WAKELOCK_DURATION_MAX+ 20000) //50s + 20s
...
int get_ssc_recovery_command(int type)
{
	...
	case SNS_RECOVERY_SSR:
		...
		break;
	case SNS_RECOVERY_CRASH:
		ret = OPTION_TYPE_SSC_FORCE_CRASH;
		break;
...
#endif
#if IS_ENABLED(CONFIG_SUPPORT_TRIPLE_6AXIS)
	int sensor_type[SENSOR_DUMP_CNT] = { MSG_LIGHT, MSG_ACCEL_SUB2, MSG_ACCEL_SUB, MSG_ACCEL, MSG_MAG_SUB, MSG_MAG, MSG_PRESSURE };
#elif IS_ENABLED(CONFIG_SUPPORT_DUAL_6AXIS)
...
void ssc_wakelock_checker(struct adsp_data *data, struct wakeup_source *ws)
{
	...
	if (ws->active) {
		...
		if (duration_ms > WAKELOCK_DURATION_MAX &&
			...
			pr_err("smp2p-sleepstate has been acquired for too long\n");
			send_ssc_recovery_command(SNS_RECOVERY_CRASH);
#else
