...
static struct device_attribute sec_battery_attrs[] = {
	...
	SEC_BATTERY_ATTR(hmap_logging_control),
	SEC_BATTERY_ATTR(hmap_logging_data),
#endif
};
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_RESET_SOC:
		break;
#if defined(CONFIG_SEC_HEATMAP_LOGGING)
	case HMAP_LOGGING_CONTROL:
		psy_do_property("battery", get,
						POWER_SUPPLY_EXT_PROP_HMAP_LOGGING_CONTROL, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
				value.intval);
		break;
	case HMAP_LOGGING_DATA:
		{
			int j = 0;
			if (!battery->hmap_buffer) {
				return -ENODATA;
			}
			for (j = 0; j < HMAP_BUFFER_SIZE; j++) {
				snprintf(temp_buf + strlen(temp_buf), MAX_BUF_SIZE - strlen(temp_buf), "%d %d %d %d %d %d %lld %d\n",
					battery->hmap_buffer[j].voltage_now,
					battery->hmap_buffer[j].current_avg,
					battery->hmap_buffer[j].batt_temp,
					battery->hmap_buffer[j].capacity,
					battery->hmap_buffer[j].fg_full_voltage,
					battery->hmap_buffer[j].recharging_voltage,
					battery->hmap_buffer[j].timestamp,
					battery->hmap_buffer[j].cc_info);
			}
					...
					const char *buf, size_t count)
{
	...
	case HMAP_LOGGING_CONTROL:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			if (x == 1) {
				value.intval = 1;
				dev_info(battery->dev, "Heatmap enable logging %d \n",value.intval);
			} else {
				value.intval = 0;
				dev_info(battery->dev, "Heatmap disable logging %d \n",value.intval);
			}
