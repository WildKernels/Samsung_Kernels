...
#if defined(CONFIG_SEC_HEATMAP_LOGGING)
#include "sec_hmap_logging.h"
#endif
...
#endif
{
	...
	if (en == AP_WAKE_INIT) {
		...
		if (!np) {
			...
			ret = of_property_read_u32(np, "battery,ap_wake_chg",
					&en);
			if (ret) {
				en = AP_WAKE_NONE;
				pr_info("%s : doesn't support ap_wake_chg\n", __func__);
			}
		}
		pr_info("%s: en: %d\n", __func__, en);
	}
...
static bool is_before_2nd_topoff(int batt_status, int chg_mode, bool is_rechg)
{
	if (batt_status == POWER_SUPPLY_STATUS_CHARGING)
		return true;

	if (batt_status == POWER_SUPPLY_STATUS_FULL && chg_mode == SEC_BATTERY_CHARGING_2ND && !is_rechg)
		return true;

	return false;
}

__visible_for_testing void sb_set_vbus_wake(struct wakeup_source *vbus_ws, int health,
		int cable_type, int batt_status, int chg_mode, bool is_rechg)
{
	int ap_wake_chg = chk_ap_wake_chg();

	if (ap_wake_chg == AP_WAKE_DAEMON_MONITOR) {
		if (!sec_bat_get_lpmode() && 
			is_before_2nd_topoff(batt_status, chg_mode, is_rechg)) {
			__pm_stay_awake(vbus_ws);
			pr_info("%s : AP_WAKE_DAEMON_MONITOR\n", __func__);
			return;
		}
	} else if (ap_wake_chg == AP_WAKE_NO_FW_PDIC) {
		if (!chg_can_sleep_type(cable_type) && (health != POWER_SUPPLY_EXT_HEALTH_UNDERVOLTAGE) &&
			(health != POWER_SUPPLY_HEALTH_OVERVOLTAGE)) {
			__pm_stay_awake(vbus_ws);
			pr_info("%s : AP_WAKE_NO_FW_PDIC\n", __func__);
			return;
		}
	}

	__pm_wakeup_event(vbus_ws, jiffies_to_msecs(HZ * 10));
	pr_info("%s : AP_WAKE_NONE\n", __func__);
}
...
static bool sec_bat_ovp_uvlo_result(struct sec_battery_info *battery, int health)
{
	...
	if (battery->health != health) {
		...
		sb_set_vbus_wake(battery->vbus_ws, health, battery->cable_type,
			battery->status, battery->charging_mode, battery->is_recharging);
		switch (health) {
		case POWER_SUPPLY_HEALTH_GOOD:
...
/* Skip notify from BC1.2 if PDIC is attached already */
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_AICL_CURRENT:
			...
			break;
#if defined(CONFIG_SEC_HEATMAP_LOGGING)
		case POWER_SUPPLY_EXT_PROP_HMAP_LOGGING_CONTROL:
			sec_hmap_set_logging_control(battery, val->intval);
			break;
#endif
...
#if defined(CONFIG_SEC_HEATMAP_LOGGING)
		case POWER_SUPPLY_EXT_PROP_HMAP_LOGGING_CONTROL:
			val->intval = sec_hmap_get_buf_status(battery);
			break;
#endif
