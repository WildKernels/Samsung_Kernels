...
#if defined(CONFIG_SEC_DUAL_FG_DYNAMIC_SCALING) && defined(CONFIG_SEC_DUAL_FG_SW_VEMPTY)
static unsigned int __read_mostly lpcharge;
module_param(lpcharge, uint, 0444);

static unsigned int sec_dual_fuelgauge_get_lpmode(void) { return lpcharge; }
#endif
...
#if defined(CONFIG_SEC_DUAL_FG_DYNAMIC_SCALING) && defined(CONFIG_SEC_DUAL_FG_SW_VEMPTY)
static void sec_dual_fuelgauge_get_vnow(struct sec_dual_fuelgauge_info *fg)
{
	if (fg->main_test_voltage <= 0) {
		fg->main_voltage_now = sec_dual_fuelgauge_get_voltage(fg, SEC_DUAL_BATTERY_MAIN);
	} else {
		fg->main_voltage_now = fg->main_test_voltage;
	}

	if (fg->sub_test_voltage <= 0) {
		fg->sub_voltage_now = sec_dual_fuelgauge_get_voltage(fg, SEC_DUAL_BATTERY_SUB);
	} else {
		fg->sub_voltage_now = fg->sub_test_voltage;
	}
}

static void sec_dual_fuelgauge_check_valert(struct sec_dual_fuelgauge_info *fg)
{
	union power_supply_propval value = {0, };

	sec_dual_fuelgauge_get_vnow(fg);

	if (!sec_dual_fuelgauge_get_lpmode() && !fg->is_charging &&
			(fg->main_voltage_now <= fg->sw_v_empty_voltage) && (fg->sub_voltage_now <= fg->sw_v_empty_voltage)) {
		pr_info("%s: Battery Voltage is Very Low!! V EMPTY(%d)\n",
			__func__, fg->vempty_mode);

		if (fg->vempty_mode != VEMPTY_MODE_HW)
			fg->vempty_mode = VEMPTY_MODE_SW_VALERT;
		else if (!fg->valert_count_flag) {
			value.intval = fg->vempty_mode;
			psy_do_property("battery", set,
					POWER_SUPPLY_PROP_VOLTAGE_MIN, value);
			fg->valert_count_flag = true;
		}
	} else if ((fg->vempty_mode == VEMPTY_MODE_SW_VALERT) &&
			((fg->main_voltage_now >= fg->sw_v_empty_recover_voltage) && (fg->sub_voltage_now >= fg->sw_v_empty_recover_voltage))) {
		fg->vempty_mode = VEMPTY_MODE_SW_RECOVERY;
		pr_info("%s: Recoverd from SW V EMPTY Activation\n", __func__);
		if (fg->valert_count_flag) {
			pr_info("%s: Vnow_main(%d), Vnow_sub(%d) release CISD VALERT COUNT check\n",
					__func__, fg->main_voltage_now, fg->sub_voltage_now);
			fg->valert_count_flag = false;
		}
	}
}
#endif
...
#if defined(CONFIG_SEC_DUAL_FG_DYNAMIC_SCALING)
#if defined(CONFIG_SEC_DUAL_FG_SW_VEMPTY)
static bool sec_dual_fuelgauge_check_vempty_recover_time(struct sec_dual_fuelgauge_info *fuelgauge)
{
...
static void sec_dual_fuelgauge_set_vempty(struct sec_dual_fuelgauge_info *fuelgauge,
				   int vempty_mode)
{
	union power_supply_propval value;
	...
	/* set vempty mode*/
	value.intval = vempty_mode;
	psy_do_property(fuelgauge->pdata->main_fg_name, set,
		(enum power_supply_property)POWER_SUPPLY_EXT_PROP_VEMPTY_DUAL, value);

	value.intval = vempty_mode;
	psy_do_property(fuelgauge->pdata->sub_fg_name, set,
		(enum power_supply_property)POWER_SUPPLY_EXT_PROP_VEMPTY_DUAL, value);

}

static unsigned int sec_dual_fuelgauge_check_vempty_status(struct sec_dual_fuelgauge_info *fuelgauge, unsigned int soc)
{
	int temperature;
	...
	/*Use minimum temperature of main OR sub bat temp */
	temperature = fuelgauge->main_bat_temp < fuelgauge->sub_bat_temp ? fuelgauge->main_bat_temp : fuelgauge->sub_bat_temp;

	/* SW/HW V Empty setting */
	if (temperature <= (int)fuelgauge->low_temp_limit) {
		if (fuelgauge->raw_capacity <= 50) {
			if ((fuelgauge->vempty_mode != VEMPTY_MODE_HW) &&
				sec_dual_fuelgauge_check_vempty_recover_time(fuelgauge)) {
				sec_dual_fuelgauge_set_vempty(fuelgauge, VEMPTY_MODE_HW);
				fuelgauge->vempty_time = 0;
			}
		} else {
			if (fuelgauge->vempty_mode == VEMPTY_MODE_HW)
				sec_dual_fuelgauge_set_vempty(fuelgauge, VEMPTY_MODE_SW);
	...
	if (!fuelgauge->is_charging && !sec_dual_fuelgauge_get_lpmode() &&
	...
	struct sec_dual_fuelgauge_info *fuelgauge, union power_supply_propval *val)
{
	...
	if (val->intval == SEC_FUELGAUGE_CAPACITY_TYPE_RAW) {
		...
		val->intval = sec_dual_fuelgauge_check_vempty_status(fuelgauge, val->intval);
#endif
			    ...
			    union power_supply_propval *val)
{
...
#if defined(CONFIG_SEC_DUAL_FG_DYNAMIC_SCALING)
#if defined(CONFIG_SEC_DUAL_FG_SW_VEMPTY)
		if (fg->initial_update_of_alert)
			sec_dual_fuelgauge_check_valert(fg);
#endif
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_RAWSOC:
			...
			case SEC_DUAL_BATTERY_MAIN:
				psy_do_property(fg->pdata->main_fg_name, get,
							(enum power_supply_property)POWER_SUPPLY_EXT_PROP_RAWSOC, value);
				val->intval = value.intval;
				break;
			case SEC_DUAL_BATTERY_SUB:
				psy_do_property(fg->pdata->sub_fg_name, get,
				...
				break;
			default:
				val->intval = (fg->main_repcap + fg->sub_repcap) * 1000 / fg->fullcaprep;
				break;
			}
			break;
		...
		case POWER_SUPPLY_EXT_PROP_SUB_ENERGY_FULL:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_VOLTAGE_NOW_TEST:
			val->intval = fg->main_test_voltage;
			break;
		case POWER_SUPPLY_EXT_PROP_SUB_VOLTAGE_NOW_TEST:
			val->intval = fg->sub_test_voltage;
			break;
		default:
			return -EINVAL;
		}
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
	{
		...
		fg->main_bat_temp = val->intval;
#if defined(CONFIG_SEC_DUAL_FG_DYNAMIC_SCALING) && defined(CONFIG_SEC_DUAL_FG_SW_VEMPTY)
		if (!fg->vempty_init_flag)
			fg->vempty_init_flag = true;
#endif
	}
		break;
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_SUB_BAT_TEMP:
			fg->sub_bat_temp = val->intval;
			break;
		case POWER_SUPPLY_EXT_PROP_VOLTAGE_NOW_TEST:
			fg->main_test_voltage = val->intval;
			break;
		case POWER_SUPPLY_EXT_PROP_SUB_VOLTAGE_NOW_TEST:
			fg->sub_test_voltage = val->intval;
			break;
		default:
			return -EINVAL;
		}
		...
		struct sec_dual_fuelgauge_info *fg)
{
	...
	if (!np) {
		...
		fg->using_temp_compensation = of_property_read_bool(np,
					"fuelgauge,using_temp_compensation");
		if (fg->using_temp_compensation) {
			ret = of_property_read_u32(np, "fuelgauge,low_temp_limit",
						 &fg->low_temp_limit);
			if (ret < 0) {
				pr_err("%s: error reading low temp limit %d\n",
					__func__, ret);
				fg->low_temp_limit = 0; /* Default: 0'C */
			}

			ret = of_property_read_u32(np,
				"fuelgauge,vempty_recover_time", &fg->vempty_recover_time);
			if (ret < 0) {
				pr_err("%s: error reading vempty_recover_time %d\n",
					__func__, ret);
				fg->vempty_recover_time = 0; /* Default: 0 */
			}
		}

		fg->using_hw_vempty = of_property_read_bool(np,
			"fuelgauge,using_hw_vempty");
		if (fg->using_hw_vempty) {
			ret = of_property_read_u32(np, "fuelgauge,sw_v_empty_voltage",
				&fg->sw_v_empty_voltage);
			if (ret < 0)
				pr_err("%s: error reading sw_v_empty_default_vol %d\n",
					__func__, ret);

			ret = of_property_read_u32(np, "fuelgauge,sw_v_empty_recover_voltage",
					&fg->sw_v_empty_recover_voltage);
			if (ret < 0)
				pr_err("%s: error reading sw_v_empty_recover_vol %d\n",
					__func__, ret);

			pr_info("%s : SW V Empty (%d)mV,  SW V Empty recover (%d)mV\n",
				__func__, fg->sw_v_empty_voltage, fg->sw_v_empty_recover_voltage);
		}
	}
	return 0;
}
...
#if defined(CONFIG_SEC_DUAL_FG_DYNAMIC_SCALING) && defined(CONFIG_SEC_DUAL_FG_SW_VEMPTY)
	fg->initial_update_of_alert = false;
	/* SW/HW init code. SW/HW V Empty mode must be opposite ! */
	fg->vempty_init_flag = false;	/* default value */
	pr_info("%s: SW/HW V empty init\n", __func__);
	sec_dual_fuelgauge_set_vempty(fg, VEMPTY_MODE_SW);
	fg->valert_count_flag = false;
#endif
