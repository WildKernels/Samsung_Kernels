...
#if IS_ENABLED(CONFIG_DUAL_BATTERY)
	if (battery->pdata->sc_vbat_thresh_main != battery->pdata->dc_step_chg_cond_vol[age_step][battery->dc_step_chg_step-1]) {
		battery->pdata->sc_vbat_thresh_main = battery->pdata->dc_step_chg_cond_vol[age_step][battery->dc_step_chg_step-1];
		pr_info("%s : set aged sc_vbat_thresh_main(%dmV)\n", __func__, battery->pdata->sc_vbat_thresh_main);
	}
	if (battery->pdata->sc_vbat_thresh_sub != battery->pdata->dc_step_chg_cond_vol_sub[age_step][battery->dc_step_chg_step-1]) {
		battery->pdata->sc_vbat_thresh_sub = battery->pdata->dc_step_chg_cond_vol_sub[age_step][battery->dc_step_chg_step-1];
		pr_info("%s : set aged sc_vbat_thresh_sub(%dmV)\n", __func__, battery->pdata->sc_vbat_thresh_sub);
	}
#endif
