...
static unsigned char page_data_buf2[4][32];
static bool pageLockStatus[4] = {true, false, false, false};
static bool pageLockStatus2[4] = {true, false, false, false};
static bool pageBufDirty[4];
static bool pageBufDirty2[4];
...
{
	int err = 0;
	bool check_slavepcb	= true;

	// Read project and region specific slavepcb value
	// For projects where slavepcb method is not used, skip
	err = of_property_read_u32(np, "samsung,eur_slavepcb", &eur_slavepcb);
	if (err) {
		check_slavepcb = false;
		auth_err("%s: 'samsung,eur_slavepcb' not found\n", __func__);
	}
	if (check_slavepcb) {
		/*  Do not proceed if
		...
		if (slavepcb != eur_slavepcb &&
			!is_sales_code("TUR")) {
			auth_info("%s Neither EUR device nor TUR region, return\n", __func__);
			return -1;
		}
	}
	return 0;
	...
	struct sec_auth_ds28e30_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		ret = of_property_read_u32_array(np, "ds28e30,offset2", pdata->control_and_data_offset2, 2);
		if (ret < 0) {
