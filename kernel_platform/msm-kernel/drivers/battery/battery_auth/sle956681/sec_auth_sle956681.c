...
Send_NVM_Status:
	...
	case SWI_NVM_SYNC:
		...
		/* Sync buffer to memory : Pages 8 to 17 */
...
static int authon_driver_parse_dt(struct authon_driver_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (ret) {
			pr_err("%s: Error! Could not read 'authon_1tau_value2'\n", __func__);
			return -1;
		}
		if (strcmp(power_mode, "indirect") == 0) {
		...
		if (ret) {
			pr_err("%s: Error! Could not read 'authon_3tau_value2'\n", __func__);
			return -1;
		}
		...
		if (ret) {
			pr_err("%s: Error! Could not read 'authon_5tau_value2'\n", __func__);
			return -1;
		}
...
static int __init authon_driver_init(void)
{
	...
	pr_info(">> %s \n", __func__);
	pr_info("driver init called and running on CPU: %d \n", get_cpu());
