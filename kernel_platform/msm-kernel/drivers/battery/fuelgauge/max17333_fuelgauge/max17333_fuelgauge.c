...
static int max17333_get_rawsoc(struct max17333_fg_data *fg, int *val)
{
	int ret;
	int repcap = -1, fullcaprep = -1;

	ret = max17333_read_capacity(fg, REG_REPCAP, &repcap);
	ret = max17333_read_capacity(fg, REG_FULLCAPREP, &fullcaprep);

	*val = (repcap * 1000) / fullcaprep;
	return ret;
}
#endif
...
#if IS_ENABLED(CONFIG_DUAL_FUELGAUGE)
static int max17333_fg_set_vempty(struct max17333_fg_data *fg,
				   int vempty_mode)
{
	struct max17333_fg_platform_data *pdata = fg->pdata;
	int ret = 0;
	...
	case VEMPTY_MODE_SW:
		/* HW Vempty Disable */
		ret = max17333_write(fg->regmap_shdw, REG_NVEMPTY_SHDW, pdata->v_empty_origin);
		if (ret < 0) {
			pr_err("%s[%s] : fail to write REG_VEMPTY to disable HW Vempty\n", __func__, max17333_battery_type[fg->battery_type]);
			return ret;
		}

		pr_info("%s: HW V EMPTY Disable, SW V EMPTY Enable with 0x%x mV\n",
			__func__, pdata->v_empty_origin);
		break;
	default:
		/* HW Vempty Enable */
		ret = max17333_write(fg->regmap_shdw, REG_NVEMPTY_SHDW, pdata->v_empty);
		if (ret < 0) {
			pr_err("%s[%s] : fail to write REG_VEMPTY to enable HW Vempty\n", __func__, max17333_battery_type[fg->battery_type]);
			return ret;
		}

		pr_info("%s: HW V EMPTY Enable, SW V EMPTY Disable 0x%x\n",
			__func__, pdata->v_empty);
		break;
	}
		...
		enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_RAWSOC:
			ret = max17333_get_rawsoc(fg, &val->intval);
			pr_info("%s: BC32 Raw SOC by Repcap(%d)\n", __func__, val->intval);
			if (ret < 0)
				return ret;
			break;
#endif
								 ...
								 const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
	{
		u16 filtercfg_val_before, filtercfg_val_after, filtercfg_val, reg_data;

		if (val->intval < 0) {
			max17333_read(fg->regmap_shdw, REG_NMANFCTRNAME1_SHDW, &reg_data);
			if (reg_data == 0) {
				max17333_write(fg->regmap_shdw, REG_NMANFCTRNAME1_SHDW,
						(reg_data + 3));
				pr_info("%s: set the low temp reset! temp : %d, capacity : 0x%x\n",
					__func__, val->intval, (reg_data + 3));
			}
		}
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_VEMPTY_DUAL:
			ret = max17333_fg_set_vempty(fg, val->intval);
			if (ret < 0)
				dev_err(fg->dev, "[%s] vempty set fail : %d\n",
						max17333_battery_type[fg->battery_type], ret);
		break;
#endif
		default:
...
static int max17333_fg_parse_dt(struct max17333_fg_data *fg)
{
	...
	pr_info("%s[%s] v_empty\n", __func__, max17333_battery_type[fg->battery_type]);
	ret = of_property_read_u32(np, "v_empty", &pval); /* v_empty */
	if (!ret)
		pdata->v_empty = pval;
	else
		pr_err("%s: error reading v_empty %d\n", __func__, ret);

	pr_info("%s[%s] v_empty_origin\n", __func__, max17333_battery_type[fg->battery_type]);
	ret = of_property_read_u32(np, "v_empty_origin", &pval); /* v_empty_origin */
	if (!ret)
		pdata->v_empty_origin = pval;
	else
