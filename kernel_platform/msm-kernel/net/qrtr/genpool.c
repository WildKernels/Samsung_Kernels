...
#define GP_DBG(x,...)	\
do {	\
		pr_err("[GP][%s]"x, __func__, ##__VA_ARGS__); \
	} while (0)
...
static void qrtr_genpool_setup_work(struct work_struct *work)
{
	...
	dev_err(qdev->dev, "@@@: %s: enter\n", __func__);
	mutex_lock(&qdev->setup_lock);
	spin_lock_irqsave(&qdev->lock, flags);
	switch (qdev->state) {
	case LOCAL_STATE_DEFAULT:
		GP_DBG(" setup work:LOCAL_STATE_DEFAULT %d\n",qdev->state);
		state_next = LOCAL_STATE_INIT;
		break;
	case LOCAL_STATE_INIT:
		GP_DBG(" setup work:LOCAL_STATE_INIT %d\n",qdev->state);
		state_next = LOCAL_STATE_START;
		break;
	case LOCAL_STATE_START:
		GP_DBG(" setup work:LOCAL_STATE_START %d\n",qdev->state);
		state_next = LOCAL_STATE_INIT;
		goto state_change;
	case LOCAL_STATE_PREPARE_REBOOT:
		GP_DBG(" setup work:LOCAL_STATE_PREPARE_REBOOT %d\n",qdev->state);
		dev_err(qdev->dev, "@@@: received setup work in LOCAL_STATE_PREPARE_REBOOT\n");
		dev_err(qdev->dev, "@@@: setting state to LOCAL_STATE_REBOOT\n");
	...
	case LOCAL_STATE_REBOOT:
		GP_DBG(" setup work:LOCAL_STATE_REBOOT %d\n",qdev->state);
		goto unlock;
	default:
	...
	GP_DBG(" setup work:qrtr_genpool_memory_free started \n");
	qrtr_genpool_memory_free(qdev);
	dev_err(qdev->dev, "calling genpool_memory_alloc\n");
	rc = qrtr_genpool_memory_alloc(qdev);
	if (rc)
		goto setup_unlock;
	GP_DBG(" setup work:qrtr_genpool_fifo_init started \n");
	...
	spin_lock_irqsave(&qdev->lock, flags);
state_change:
	dev_err(qdev->dev, "@@@: setting state to state_next\n");
	qrtr_genpool_set_state(qdev, state_next);
	qrtr_genpool_signal_setup(qdev);
	dev_err(qdev->dev, "@@@: %s: unlock\n", __func__);
				  ...
				  unsigned long action, void *data)
{
	...
	switch (qdev->state) {
	case LOCAL_STATE_START:
		dev_err(qdev->dev, "@@@: setting state to LOCAL_STATE_PREPARE_REBOOT\n");
...
static int qrtr_genpool_probe(struct platform_device *pdev)
{
	...
	mutex_lock(&qdev->setup_lock);
	spin_lock_irqsave(&qdev->lock, flags);
	if (qdev->state == LOCAL_STATE_DEFAULT)
	    schedule_work(&qdev->setup_work);
	spin_unlock_irqrestore(&qdev->lock, flags);
	mutex_unlock(&qdev->setup_lock);
